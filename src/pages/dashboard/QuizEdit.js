...
const QuizEdit = () => {
  ...
  const { video_title, question: getQuestion, video_id, options } = quiz || {};
  ...
  useEffect(() => {
    ...
    setVideoId(video_id);
    setOptionOne((prev) => quizOptions[0] || prev);
    setOptionTwo((prev) => quizOptions[1] || prev);
    setOptionThree((prev) => quizOptions[2] || prev);
    setOptionFour((prev) => quizOptions[3] || prev);
  }, [video_title, getQuestion, video_id, options]);
